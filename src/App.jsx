  ...
  return (
    <div style={{ position: "relative", height: 28, background: "#1a1a2e", borderRadius: 6, overflow: "hidden", marginTop: 6 }}>
      ...
      <div style={{
        ...
        height: "100%", background: "rgba(255,255,255,0.07)", borderLeft: "1px dashed rgba(255,255,255,0.2)", borderRight: "1px dashed rgba(255,255,255,0.2)"
      }} />
      {/* actual bar */}
      <div style={{
...
export default function App() {
  ...
  async function getAdvice() {
    ...
    const breakdown = BUCKET_KEYS.map(k => {
      const [min, max] = targets[k];
      const status = statuses[k] ? "範囲内" : pcts[k] < min ? "不足" : "超過";
      return `・${BUCKET_META[k].label}（${BUCKET_META[k].desc}）: ${formatMan(amounts[k])} / ${pcts[k].toFixed(1)}% → 目標 ${min}–${max}% [${status}]`;
    }).join("\n");

    const prompt = `あなたは資産運用のアドバイザーです。以下のポートフォリオ状況を見て、具体的で実践的なリバランスのアドバイスを日本語で300字以内で答えてください。

現在年齢：${age}歳
目標フェーズ：${age === 55 ? "55歳→60歳移行期" : "60歳以降・守りと維持期"}
総資産：${formatMan(total)}
${breakdown}

・ズレているバケットに絞って、何をどう動かすかを具体的に教えてください
・専門用語は避け、平易な言葉で
・不安を煽らず、落ち着いたトーンで`;
    ...
    try {
      ...
      const text = data.content?.filter(b => b.type === "text").map(b => b.text).join("") || "アドバイスを取得できませんでした。";
      setAdvice(text);
    } catch (e) {
      setAdvice("通信エラーが発生しました。");
    }
    setLoading(false);
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ flex: 1 }}>
          <label style={{ fontSize: 10, color: "#888", display: "block", marginBottom: 4, letterSpacing: 2 }}>現在の年齢フェーズ</label>
      ...
      {hasData && (
        <div style={{
          ...
          <div style={{ marginTop: 8, display: "flex", justifyContent: "center", gap: 12 }}>
            {BUCKET_KEYS.map(k => (
              <span key={k} style={{ fontSize: 10, color: statuses[k] ? "#4ecca3" : "#e74c3c" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 24 }}>
        {BUCKET_KEYS.map(k => {
          ...
          return (
            <div key={k} style={{
              background: "#13131f", border: `1px solid ${inRange && hasData ? "rgba(78,204,163,0.2)" : hasData ? "rgba(231,76,60,0.2)" : "#1e1e32"}`,
      ...
      {hasData && (
        <button onClick={getAdvice} disabled={loading} style={{
          ...
          {loading ? "分析中…" : allOk ? "✓ バランス良好 — AIに確認する" : "⚠ ズレあり — AIにアドバイスを求める"}
        </button>
      )}
      ...
      {advice && (
        <div style={{
          ...
          <div style={{ fontSize: 10, letterSpacing: 3, color: "#c8a96e", marginBottom: 12 }}>AI ADVISOR</div>
          <p style={{ fontSize: 14, lineHeight: 1.8, color: "#d0c8b8", margin: 0, whiteSpace: "pre-wrap" }}>{advice}</p>
        </div>
      )}
